...
function DoctorImage({ src, alt }: { src: string; alt: string }) {
    const [isLoaded, setIsLoaded] = useState(false);

    return (
        <div className="w-full md:w-2/5 aspect-[4/5] relative rounded-3xl overflow-hidden shadow-2xl bg-gray-100 transition-transform duration-500 group-hover:scale-[1.02]">
            ...
            <Image
                ...
                priority={true}
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#191F28]/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-20" />
        </div>
    );
}
            ...
            <div className="max-w-7xl mx-auto px-6">
                ...
                <div className="grid grid-cols-1 gap-24">
                    {DOCTORS.map((doctor, index) => (
                        ...
                        >
                            <DoctorImage src={doctor.imageSrc} alt={doctor.name} />
                            ...
                            <div className="w-full md:w-3/5 flex flex-col justify-center">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <button className="bg-[#191F28] text-white py-4 rounded-xl font-bold hover:bg-[#333D4B] transition-all cursor-pointer">진료 예약하기</button>
                                    <button className="bg-white text-[#191F28] py-4 rounded-xl font-bold border border-[#F2F4F6] hover:bg-gray-50 transition-all cursor-pointer">상세 약력보기</button>
                                </div>
                            </div>
                        </div>
                    ))}
